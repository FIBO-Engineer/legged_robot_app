...
//       final groundMaterial = three.MeshLambertMaterial.fromMap({
//         'color': 0xFF171F2B,
//       });
...
//           0.3; // ค่าต่ำ = zoom ช้าและละเอียดขึ้น (default = 1.0)
...
//       talker.info(
//         'ThreeJS not yet initialized, storing size update: ${width}x$height',
//       );
//       return;
//     }
...
//       talker.info(
//         'Robot loaded successfully with ${newRobot.links.length} links',
//       );
//     } catch (e) {
...
//     final joint = robot.value!.joints.firstWhere(
//       (j) => j.name == jointName,
//       orElse: () => robot.value!.joints.first,
//     );

//     // หา child link
//     final childLink = robot.value!.links.firstWhere(
//       (link) => link.data.name == joint.child,
//       orElse: () => robot.value!.links.first,
//     );
...
//       final highlightMaterial = three.MeshPhongMaterial.fromMap({
//         'color': 0xFF1F2733,
//         'transparent': false,
//         'shininess': 30,
//         'specular': 0x111111,
...
//         leftArmOrder.map<double>((name) => getJointValue(name)).toList();
//     final List<double> rightPositions =
//         rightArmOrder.map<double>((name) => getJointValue(name)).toList();
